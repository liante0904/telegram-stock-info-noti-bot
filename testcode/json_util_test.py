...
def format_message(data):
    ...
    ARTICLE_TITLE = data.get('ARTICLE_TITLE', 'No Title')
    ARTICLE_URL = data.get('ATTACH_URL', 'No URL')
    ...
    # 조건에 따라 메시지 끝에 개행 문자 추가
    if ('네이버' in data['FIRM_NM'] or '조선비즈' in data['FIRM_NM']):
        sendMessageText += '\n'

    return sendMessageText


def format_message(unsent_data):
    messages = []
    current_message = ""
