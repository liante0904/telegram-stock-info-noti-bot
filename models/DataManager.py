...
class DataManager:
    ...
    async def daily_update_data(self, date_str=None, fetched_rows=None, type=None):
        ...
        try:
            # 통합된 OracleManager의 daily_update_data 호출
            await self.oracle.daily_update_data(fetched_rows=fetched_rows, type=type, date_str=date_str)
        except Exception as e:
    ...
    async def update_report_summary(self, record_id, summary, model_name, telegram_url=None):
        ...
        try:
            ...
            self.logger.error(f"SQLite Summary Update Error: {str(e)}")
        ...
        try:
            ...
            self.logger.error(f"Oracle Summary Update Error: {str(e)}")
