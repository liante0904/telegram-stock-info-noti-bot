...
class SyncWebScraper:
    ...
    def Get(self, params=None):
        ...
        logger.debug(f"WebScraper Get successful: {self.target_url}")
    ...
    def GetJson(self, params=None):
        ...
        try:
            ...
            logger.debug(f"WebScraper GetJson: {self.target_url}")
            ...
            try:
                ...
                with open("error_response.txt", "w", encoding="utf-8") as f:
                    f.write(raw_text)
                logger.debug("[디버깅] 원본 응답이 error_response.txt에 저장되었습니다.")
                return None
...
class AsyncWebScraper:
    ...
    async def GetJson(self, session=None, params=None):
        ...
        try:
            async with session.get(self.target_url, headers=self.headers, params=params) as response:
                response.raise_for_status()
                logger.debug(f"AsyncWebScraper GetJson successful: {self.target_url}")
