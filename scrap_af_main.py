...
async def update_firm_telegram_url_by_date(date_str=None):
    ...
    """
    logger.info('Starting update_firm_telegram_url_by_date')
    db = SQLiteManager()
    ...
    for sec_firm_order in range(len(FirmInfo.firm_names)):
        ...
        if firm_info.get_firm_name() and firm_info.telegram_update_required:
            ...
            if records:
                logger.info(f"Fetched {len(records)} records for {firm_info.get_firm_name()} (SEC_FIRM_ORDER {sec_firm_order})")
                all_records.extend(records)

                # 조건에 따라 추가 작업 수행
                if sec_firm_order == 19:
                    # sec_firm_order가 19인 경우 업데이트 수행
                    logger.info(f"Updating TELEGRAM_URL for records with SEC_FIRM_ORDER 19 ({firm_info.get_firm_name()})")
                    ...
                    for record in update_records:
                        ...
                        logger.debug(f"Updated TELEGRAM_URL for report_id {record['report_id']}")
                
                elif sec_firm_order == 0:
                    # sec_firm_order가 0인 경우 추가 작업 수행
                    logger.info(f"Additional processing for SEC_FIRM_ORDER 0 ({firm_info.get_firm_name()})")
                    for record in records:
                        ...
                        logger.debug(f"LS Detail update result: {r}")

    # 전체 회사들의 레코드가 JSON 리스트로 모임
    if all_records:  # all_records가 비어 있지 않은 경우에만 JSON 변환 및 출력 수행
        logger.info(f"Combined {len(all_records)} records from all firms.")

# 메인 함수
async def main():
    logger.info('=================== scrap_af_main START ===================')
