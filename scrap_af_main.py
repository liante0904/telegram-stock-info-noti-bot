...
async def update_firm_telegram_url_by_date(date_str=None):
    ...
    for sec_firm_order in range(len(FirmInfo.firm_names)):
        ...
        if firm_info.get_firm_name() and firm_info.telegram_update_required:
            ...
            print(f"Fetched records for SEC_FIRM_ORDER {sec_firm_order}: {len(records)}")
            if records:
                all_records.extend(records)
            else:
                print("TODO Record empty")
                continue
            ...
            if sec_firm_order == 19:
                ...
                for record in update_records:
                    await db.update_telegram_url(record_id=record['id'], telegram_url=record['TELEGRAM_URL'])
                ...
                for record in update_records:
                    await db.update_telegram_url(record_id=record['id'], telegram_url=record['TELEGRAM_URL'], article_title=record['ARTICLE_TITLE'])
                    print(f"Updated TELEGRAM_URL for id {record['id']} with {record['TELEGRAM_URL']}")
