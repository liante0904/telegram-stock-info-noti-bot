...
- **전환 시 처리 항목:**
  ...
  - [x] `scraper.py` 및 모듈에서 `get_db()` (DB_BACKEND env 기반 팩토리) 사용
  - [x] FirmInfo PostgreSQL 전환 (DB_BACKEND=postgres 시 psycopg2로 TBM_SEC_FIRM_INFO 로드)
...
- [x] **ADR-003** `secrets.json` URL 관리 + ConfigManager `get_urls()` 추가 + 모듈 교체
- [x] **ADR-004** PostgreSQL DDL 적용 및 데이터 이전 (270,914 rows)
- [x] **ADR-004** `PostgreSQLManager` SQLiteManager 호환 인터페이스 구현
- [x] **ADR-004** `scraper.py` 및 모듈에서 `get_db()` 팩토리 사용 (DB_BACKEND=postgres로 전환 가능)
