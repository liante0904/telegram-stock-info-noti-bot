...
스크래퍼 (scraper.py)
    │
    ▼
PostgreSQL (TB_SEC_REPORTS)     ← DB_BACKEND=postgres (2026-04-18 전환 완료)
    │
    ├── pgAdmin4 (웹 GUI)
    └── 텔레그램 채널 발송
...
- **결정:** PostgreSQL로 완전 전환
- **롤백:** `DB_BACKEND=sqlite` 환경변수 변경 후 재시작으로 즉시 롤백 가능
- **전환 항목:**
  - [x] PostgreSQL DDL 적용 (`TB_SEC_REPORTS`, `TBM_SEC_FIRM_INFO`, `TBM_SEC_FIRM_BOARD_INFO`)
  - [x] 데이터 이전 완료 (270,914 rows, idempotent batch 5000)
  - [x] `models/PostgreSQLManager.py` — SQLiteManager 호환 인터페이스
  - [x] `db_factory.get_db()` — DB_BACKEND 기반 팩토리
  - [x] FirmInfo PostgreSQL 전환 (DB_BACKEND=postgres 시 psycopg2로 로드)
  - [x] 프로덕션 `DB_BACKEND=postgres` 전환 완료
...
- **상태:** 예정
- **배경:** PostgreSQL 전환으로 DB 레이어 인터페이스가 두 개(SQLite/PostgreSQL)가 됨 → 동등성 보장 필요
- **범위:**
  - `ConfigManager` — dev/prod 분기, `get_urls()` 환경변수 폴백
  - `db_factory` — `DB_BACKEND` 환경변수별 올바른 매니저 반환
  - `SQLiteManager` ↔ `PostgreSQLManager` 동등성 — 같은 데이터 insert/select 결과 동일
  - 모듈 파싱 로직 — HTML fixture 기반 파싱 결과 검증 (외부 HTTP 요청 없이)
- **도구:** pytest, pytest-asyncio
- **제외:** 실제 사이트 요청 E2E (외부 의존성 → CI에서 불안정)
...
- [x] **ADR-004** 프로덕션 `DB_BACKEND=postgres` 전환
