...
- **목표 아키텍처:**
  ...
  ```
- **전환 시 처리 항목:**
  - `TBM_SEC_FIRM_INFO` / `TBM_SEC_FIRM_BOARD_INFO` 이전
  - `DATA_MAIN_DAILY_SEND` 이전
  - `TARGET_URL` 컬럼을 이전 시 함께 추가 (ADR-003 통합)
  - Oracle ATP / ORDS 제거
  - `models/SQLiteManager.py` → `models/PostgreSQLManager.py` 교체
...
- [ ] **ADR-004** PostgreSQL 스키마 설계 및 데이터 이전
- [ ] **ADR-004** `SQLiteManager` → `PostgreSQLManager` 교체
