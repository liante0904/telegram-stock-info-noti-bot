...
def HanKyung_checkNewArticle():
    ...
    # 하나금융 Daily
    TARGET_URL_0 =  'https://consensus.hankyung.com/analysis/list?skinType=business&pagenum=80'
    # 하나금융 산업 분석
    TARGET_URL_1 =  'https://consensus.hankyung.com/analysis/list?skinType=industry&pagenum=80'
    ...
    for ARTICLE_BOARD_ORDER, TARGET_URL in enumerate(TARGET_URL_TUPLE):
        try:
            sendMessageText += HanKyung_parse(ARTICLE_BOARD_ORDER, TARGET_URL)
        except:
            sendMessageText = ''
        if len(sendMessageText) > 3500:
            print("발송 게시물이 남았지만 최대 길이로 인해 중간 발송처리합니다. \n", sendMessageText)
...
def HanKyung_parse(ARTICLE_BOARD_ORDER, TARGET_URL):
    ...
    if ARTICLE_BOARD_ORDER == 0 : # 기업분석
        FIRST_ARTICLE_TITLE = soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td.text_l > a')[FIRST_ARTICLE_INDEX].text.strip()
        FIRST_ARTICLE_URL =  'https://consensus.hankyung.com' + soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td:nth-child(9) > div > a')[FIRST_ARTICLE_INDEX].attrs['href']
        # try:
        #     # ARTICLE_BOARD_NAME =  GetBoardName() 
        #     FIRST_ARTICLE_TITLE = soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td.text_l > a')[FIRST_ARTICLE_INDEX].text.strip()
        #     FIRST_ARTICLE_URL =  'https://consensus.hankyung.com' + soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td:nth-child(9) > div > a')[FIRST_ARTICLE_INDEX].attrs['href']
        ...
        FIRST_ARTICLE_TITLE = soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td.text_l > a')[FIRST_ARTICLE_INDEX].text.strip()
        FIRST_ARTICLE_URL =  'https://consensus.hankyung.com' + soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td:nth-child(7) > div > a')[FIRST_ARTICLE_INDEX].attrs['href']
        # try:
        #     # ARTICLE_BOARD_NAME =  GetBoardName() 
        #     FIRST_ARTICLE_TITLE = soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td.text_l > a')[FIRST_ARTICLE_INDEX].text.strip()
        #     FIRST_ARTICLE_URL =  'https://consensus.hankyung.com' + soup.select('#contents > div.table_style01 > table > tbody > tr:nth-child(1) > td:nth-child(7) > div > a')[FIRST_ARTICLE_INDEX].attrs['href']
    ...
    for list in soupList:
        ...
        if ARTICLE_BOARD_ORDER == 0 : # 기업분석
            LIST_ARTICLE_URL =  'https://consensus.hankyung.com' + list.select_one('#contents > div.table_style01 > table > tbody > tr > td:nth-child(9) > div > a').attrs['href']
            LIST_ARTICLE_BROKER_NAME =list.select_one('#contents > div.table_style01 > table > tbody > tr > td:nth-child(6)').text.strip()
        else:                          # 산업분석
            LIST_ARTICLE_URL =  'https://consensus.hankyung.com' + list.select_one('#contents > div.table_style01 > table > tbody > tr > td:nth-child(7) > div > a').attrs['href']
            LIST_ARTICLE_BROKER_NAME =list.select_one('#contents > div.table_style01 > table > tbody > tr > td:nth-child(5) > div > a').text.strip()
...
def DB_SelNxtKey(SEC_FIRM_ORDER, ARTICLE_BOARD_ORDER):
    ...
    dbQuery  = " SELECT 	BOARD_NM, SEC_FIRM_ORDER, ARTICLE_BOARD_ORDER, BOARD_URL, NXT_KEY, NXT_KEY_ARTICLE_TITLE, SEND_YN, CHANGE_DATE_TIME, TODAY_SEND_YN, TIMESTAMPDIFF(second ,  CHANGE_DATE_TIME, CURRENT_TIMESTAMP) as SEND_TIME_TERM 		FROM nxt_key		WHERE 1=1 AND  SEC_FIRM_ORDER = %s   AND ARTICLE_BOARD_ORDER = %s "
