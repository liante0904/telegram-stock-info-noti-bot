...
def get_data_from_db(table_name):
    ...
    :return: 조회된 데이터
    """
    db_path = get_file_path('db/telegram.db')
    connection = sqlite3.connect(db_path)
    cursor = connection.cursor()
    query = f"SELECT * FROM {table_name} WHERE SAVE_TIME LIKE ?"
    today_date = f"{DEFAULT_DATE}%"
...
async def main():
    ...
    table_names = [
        ...
        'hankyungconsen_research',
        'naver_research'
    ]
    
    # 데이터베이스에서 데이터 조회
    data_sources = [get_data_from_db(table) for table in table_names]

    # 데이터 필터링
    main_data_today = filter_today_data(data_sources[0])
    other_data_sources_today = [filter_today_data(data) for data in data_sources[1:]]
    ...
    # 알림 키워드 JSON 파일 경로
    relative_path = os.path.join(get_project_root(), '/json')
