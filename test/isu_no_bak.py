...
def get_stock_codes_by_name_or_code(input_value):
    ...
    # 종목명 또는 종목코드 부분 일치를 검사하여 해당하는 종목과 종목코드를 저장합니다.
    matching_stocks = []
    for ticker in tickers:
        name = stock.get_market_ticker_name(ticker)
        if input_value in name or input_value == ticker:
...
def main():
    ...
    if matching_stocks:
        ...
        # 입력값이 6자리 종목코드인지 확인
        if len(choice) == 6:
            stock_name = stock.get_market_ticker_name(choice)
            if stock_name:
                print(f"선택한 종목명: {stock_name}, 종목코드: {input_value}")
            else:
                print("입력한 종목코드를 찾을 수 없습니다.")
        else:
