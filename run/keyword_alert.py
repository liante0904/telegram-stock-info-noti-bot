...
async def run_once():
    logger.info("Fetching keywords from DB...")
    try:
        user_keywords = db_manager.load_keywords_from_db()
    ...
    for user_id, entries in user_keywords.items():
        for entry in entries:
            ...
            try:
                found_reports = db_manager.fetch_keyword_reports(date=today, keyword=keyword, user_id=user_id)
                if found_reports:
                    logger.success(f"[{user_id}] Found {len(found_reports)} reports for '{keyword}'")
                    header = f"===== 알림 키워드 : {keyword} =====\n"
                    chunks = get_report_messages(found_reports)
                    for i, chunk in enumerate(chunks):
                        msg = (header if i == 0 else "") + chunk
                        await sendMarkDownText(token=TOKEN, chat_id=user_id, sendMessageText=msg)
                    db_manager.update_keyword_send_user(date=today, keyword=keyword, user_id=user_id)
            except Exception as e:
